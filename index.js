...
console.log("Georgina Salazar Partida")
...
//llamada a la promesa#1
 miPromesa
  .then((value) => {   console.log("Promesa #1 "+value);   })
  .catch((err) => {   console.log("Promesa #1 "+err);   });
...
const promise = new Promise((resolve, reject) => {
  ...
  const number = Math.floor(Math.random() * 10);
	...
	setTimeout(
		( ) => number > 5		? resolve(number)  	: reject(new Error('Menor a 5 salio un '+number)),
		1000
	);
  ...
  function randomDelayed(max = 10, expected = 5, delay =  1000) {
    return new Promise( (resolve, reject) => {
      ...
      setTimeout(
        () => number > expected
          ? resolve(number)
          : reject(new Error('número menor a '+expected+ ' salio '+number)),
        delay
      );
    });
  }
...
function getPosts2() {
 ...
 getPosts2()
  .then( (r) => {  console.log("Promesa#5",r)  })
  .catch( (error) => {   console.log('Promesa#5 Algo salió mal ' + error);   });
